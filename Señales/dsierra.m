% dsierra -- Genera la señal diente de sierra y su vector de tiempo correspondiente.
%
%	ft = dsierra(T0, ciclos, dt)
%	ft = dsierra(T0, ciclos, dt, start)
%
% Argumentos
% ==========
%
...
%
function [xt, t] = dsierra(T0,ciclos,dt,start)
...
xt = 0;
for n = nizq:nder;
	xt = xt + rampa(t-T0*n) - rampa(t-T0*(n+1)) - T0*escalon(t-T0*(n+1));
end